...
fn wrap_peer(inner: simplex_dns::ActiveSimplexDnsPeer) -> MuxSimplexDnsPeer {
...
pub async fn connect_mux_peer(
    ...
    endpoint: &str,
) -> Result<MuxSimplexDnsPeer, Error> {
    let peer = simplex_dns::connect_peer(identity, endpoint).await?;
    Ok(wrap_peer(peer))
}
...
pub async fn accept_mux_peer_on(
    ...
    let peer = simplex_dns::accept_peer_on(identity, socket, path).await?;
    Ok(wrap_peer(peer))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn simplex_dns_mux_routes_frames_by_stream_id() {
        ...
        let server_identity = AgentIdentity::from_config(&AgentIdentityConfig {
            name: Some("simplex-dns-mux-server".into()),
            key: None,
        });
        let client_identity = AgentIdentity::from_config(&AgentIdentityConfig {
            name: Some("simplex-dns-mux-client".into()),
            key: None,
        });
