...
use crate::{
    ...
    error::{report_route_pruned, report_route_rejected},
    protocol::{
    ...
    tunnel::{
        ...
        streamhttp, tcp_mux, ws_mux, h2_mux,
    },
};
...
pub async fn send_direct_announce_h2_mux(
...
pub async fn send_route_snapshot_h2_mux(
    peer: &h2_mux::MuxH2Peer,
    identity: &AgentIdentity,
...
pub async fn broadcast_control_frame_h2(
    peer_map: &Arc<Mutex<HashMap<String, h2_mux::MuxH2Peer>>>,
    identity: &AgentIdentity,
    source_peer_id: &str,
    frame: &Frame,
) -> Result<(), Error> {
...
async fn allocate_h2_relay_stream_id(allocator: &Arc<Mutex<u32>>) -> u32 {
...
async fn bridge_h2_stream_frames(
    mut from_rx: tokio::sync::mpsc::Receiver<Frame>,
    to_peer: h2_mux::MuxH2Peer,
...
pub async fn handle_h2_relay_stream_open(
    peer_map: Arc<Mutex<HashMap<String, h2_mux::MuxH2Peer>>>,
    allocator: Arc<Mutex<u32>>,
    relay_links: RelayLinkMap,
    source_peer: h2_mux::MuxH2Peer,
    open_frame: Frame,
) -> Result<(), Error> {
    ...
    let target_stream_id = allocate_h2_relay_stream_id(&allocator).await;
    upsert_relay_link(
        &relay_links,
        "h2",
    ...
    tokio::spawn(async move {
        if let Err(err) = bridge_h2_stream_frames(
            downstream_rx,
            upstream_peer.clone(),
            target_stream_id,
...
pub async fn send_direct_announce_streamhttp(
    peer: &streamhttp::ActiveStreamHttpPeer,
    identity: &AgentIdentity,
    services: &[String],
) -> Result<(), Error> {
    let announce = Frame::new(
    ...
    match message {
        ...
        Message::RouteUpdate(update) => {
            for announcement in &update.announcements {
                ...
                {
                    report_route_rejected(&announcement.origin_agent_id, peer_agent_id, &reason);
                    continue;
                }
