...
use crate::{
    ...
    tunnel::{simplex_http_mux, tcp_mux, ws_mux},
};
use tokio::{
    io::AsyncReadExt,
...
pub async fn run_outbound_socks5_simplex_http_once(
    identity: AgentIdentity,
    endpoints: &[TunnelEndpoint],
    local_socks_definition: ServiceDefinition,
    remote_raw_definition: ServiceDefinition,
    ...
    loop {
        ...
        let stream_id = allocator.next();
        tokio::spawn(async move {
            if let Err(err) = handle_outbound_socks5_client_with_failover_simplex_http(
                identity,
                endpoints,
                conn_policy,
                socks_service,
...
async fn connect_selected_socks5_peer_simplex_http(
    identity: AgentIdentity,
    endpoints: &[TunnelEndpoint],
    conn_policy: &ConnPolicy,
    hub: &Arc<Mutex<SessionHub>>,
...
pub async fn handle_outbound_socks5_simplex_http_client(
    ...
    accept_auth(&mut client, &local_socks_service).await?;
    let request = read_connect_request(&mut client).await?;
    process_outbound_socks5_simplex_http_request(
        peer,
        remote_raw_definition,
        remote_peer_id,
        &mut client,
...
async fn process_outbound_socks5_simplex_http_request(
    peer: simplex_http_mux::MuxSimplexHttpPeer,
    remote_raw_definition: ServiceDefinition,
    remote_peer_id: Option<String>,
    client: &mut TcpStream,
...
async fn handle_outbound_socks5_client_with_failover_simplex_http(
    identity: AgentIdentity,
    endpoints: Vec<TunnelEndpoint>,
    conn_policy: ConnPolicy,
    local_socks_service: Socks5Service,
    ...
    Err(last_err.unwrap_or_else(|| Error::other("no socks5 simplex-http upstream succeeded")))
}
