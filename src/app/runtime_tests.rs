use crate::{
    ...
    serve::{
        ...
        raw::{
            proxy_mux_stream_loop, proxy_simplex_dns_mux_stream_loop,
            proxy_simplex_oss_mux_stream_loop, proxy_ws_mux_stream_loop, RawService,
        },
        service::{build_remote_services, ServiceDefinition},
        socks5::Socks5Service,
    },
    tunnel::{
        simplex_dns, simplex_dns_mux, simplex_http, simplex_oss, simplex_oss_mux,
        tcp::bind,
...
async fn simplex_oss_multi_hop_relay_roundtrip(relay_count: usize, stream_id: u32, payload: &[u8]) {
    ...
    match response.message {
        Message::StreamData(data) => assert_eq!(data.to_bytes().unwrap(), payload),
        other => panic!("unexpected simplex oss relayed stream response: {:?}", other),
    }
...
async fn simplex_dns_multi_hop_relay_roundtrip(relay_count: usize, stream_id: u32, payload: &[u8]) {
    let echo_listener = bind("127.0.0.1:0").await.unwrap();
    let echo_addr = echo_listener.local_addr().unwrap();
    tokio::spawn(async move {
        let (mut stream, _) = echo_listener.accept().await.unwrap();
    ...
    let target_listener = simplex_dns::bind("simplex+dns://127.0.0.1:0/target.local")
        .await
        .unwrap();
    let target_addr = target_listener.local_addr().unwrap();
    let target_identity = AgentIdentity::from_config(&AgentIdentityConfig {
    ...
    let target_id = target_identity.id.clone();
    let target_task = tokio::spawn(async move {
        let peer = simplex_dns_mux::accept_mux_peer_on(target_identity, target_listener, "/target.local")
            .await
            .unwrap();
        let (accepted_stream_id, open) = peer.read_stream_open().await.unwrap();
        let rx = peer.open_stream_receiver(accepted_stream_id).await;
    ...
    for _ in 0..relay_count {
        let socket = simplex_dns::bind("simplex+dns://127.0.0.1:0/relay.local")
            .await
            .unwrap();
    ...
    for (idx, socket) in relay_sockets.into_iter().enumerate().rev() {
        ...
        let upstream_endpoint = if idx + 1 == relay_count {
            format!("simplex+dns://{}/target.local", target_addr)
        } else {
            format!("simplex+dns://{}/relay.local", relay_addrs[idx + 1])
        };
        let target_id = target_id.clone();
        relay_tasks.push(tokio::spawn(async move {
            let downstream_peer =
    ...
    let leaf_peer = simplex_dns_mux::connect_mux_peer(
        leaf_identity,
        &format!("simplex+dns://{}/relay.local", relay_addrs[0]),
    )
    .await
    .unwrap();
    let open_frame = Frame::new(
    ...
    match response.message {
        Message::StreamData(data) => assert_eq!(data.to_bytes().unwrap(), payload),
        other => panic!("unexpected simplex dns relayed stream response: {:?}", other),
    }
...
async fn outbound_task_fails_over_to_second_tcp_endpoint() {
    ...
    let shared = RuntimeShared::new(Vec::new(), &root).await;
...
async fn simplex_dns_relay_stream_bridge_roundtrip_through_three_hops() {
    simplex_dns_multi_hop_relay_roundtrip(2, 51, b"simplex-dns-relay-3-hop-ok").await;
}
...
async fn outbound_task_over_simplex_oss_endpoint_succeeds() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        loop {
            let frame = peer.read_frame().await.unwrap();
            if matches!(frame.message, Message::AgentAnnounce(_) | Message::RouteUpdate(_)) {
                continue;
            }
            match frame.message {
...
async fn simplex_oss_inbound_runtime_registers_direct_session() {
    ...
    let listener_identity = AgentIdentity::from_config(&listener_config.identity);
    let inbound_tasks =
        spawn_inbound_tasks(&listener_config, &listener_identity, &shared, None, false, &[]).await;
    assert_eq!(inbound_tasks.len(), 1);
